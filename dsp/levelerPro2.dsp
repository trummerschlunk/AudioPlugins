...
with {
  ...
  leveler_brake_threshold_dynamic(in) = min_track(in) + expander_thresh_offset;
    ...
    expander_thresh_offset = hslider("thresh offset", 26,0,40,1);

  min_track(l) = l : min_meter with {
    min_meter = an.amp_follower_ar(0.01,0.01) : ba.slidingMin(sliding_window * ma.SR, Sliding_window_max) : si.smooth(ba.tau2pole(min_smoothing_time)) : ba.linear2db : max(-60) <: attach(_,hbargraph("min_track",-100,0));
    sliding_window = 3;
    min_smoothing_time = 1;
    
};

//expander_sc_filter = fi.highpass(2,200) : fi.lowpass(2,600);
